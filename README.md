...
|-----------|-------------|
| **Ручное тестирование** | Тест-анализ, декомпозиция требований, тест-дизайн, чек-листы, тест-кейсы, баг-репорты, кросс-браузерное тестирование, Figma |
| **Мобильное тестирование** | Эмулятор Android в Android Studio, тестирование на разных ориентациях и разрешениях экрана |
...
|--------|---------|---------:|---------------|
| **[Яндекс.Маршруты](./manual-testing/yandex-marshruty/README.md)** | Веб-приложение: вёрстка, функционал, оплата. Кросс-браузерно — Яндекс.Браузер и Firefox | 105 | **39** — 6 блокирующих, 14 критических |
| **[Яндекс.Прилавок](./manual-testing/yandex-prilavok/README.md)** | API сервиса доставки + Postman-коллекция | 110 | **60** — 1 блокирующий, 22 критических |
| **[Яндекс.Метро](./manual-testing/yandex-metro/README.md)** | Мобильное приложение на эмуляторе Android: функционал и регрессия | 76 | **19** — 2 блокирующих, 6 критических |

В каждом проекте — анализ и декомпозиция требований, техники тест-дизайна (классы эквивалентности, граничные значения, предугадывание ошибок) с разбором в отдельных таблицах, полная тестовая документация и баг-репорты.
...
| [BUG2](./bug-reports/BUG2-udalenie-kuriera-bez-id.md) | Ошибка 404 Not Found при отправке запроса `DELETE /api/v1/courier/{id}` на удаление курьера, если не передан id курьера | 🟡&nbsp;Стандартный |
| [BUG3](./bug-reports/BUG3-prinyatie-zakaza-bez-id.md) | Ошибка 404 Not Found при отправке запроса `PUT /api/v1/orders/accept/{id}` на принятие заказа курьером, если не передан id заказа | 🟡&nbsp;Стандартный |
...
Основная работа — эксплуатация: обеспечение бесперебойной работы оборудования связи на участке железной дороги.
...
**Ближе всего к тестированию.** Примерно раз в квартал, при выходе новых версий ПО оборудования связи:

- Проводила функциональную и регрессионную проверку обновлений перед установкой
- Выявляла и документировала дефекты в тестовых сборках, при критических сбоях инициировала откат на стабильную версию
- Составляла отчёты о результатах проверки
- Воспроизводила ошибки вместе с техподдержкой и разработчиками вендора
- Участвовала в установке релизов и вела пост-релизный мониторинг

*Что из этого пригодилось в QA:* разбор инцидентов на работающей системе, приоритизация по критичности отказа, внятное описание проблемы для того, кто будет её чинить, и внимание к деталям в условиях ограниченного времени.
